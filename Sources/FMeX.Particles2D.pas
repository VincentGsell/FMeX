...
Uses
  ...
  {$ENDIF}
  ,GS.ParticleEngine2D, FMeX.Types3D, FMeX.Images
  ;
...
TeParticle2d = Class(TeCustomMesh)
protected
...
begin
  ...
  begin
    p := Data.VertexBuffer.VerticesPtr[VerticeIndice];
    //Note : Above instead of "    //Data.VertexBuffer.Vertices[VerticeIndice] := ..." --> Not notable perf. difference. But I LOVE pointer. ;)
    ...
    p := Data.VertexBuffer.VerticesPtr[VerticeIndice+1];
    p^.X := FParticleEngine.ParticleList[i].x + 0.5 * epsilon;
    p^.Y := FParticleEngine.ParticleList[i].y - 0.5 * epsilon;
    p^.Z := 0;
    ...
    p := Data.VertexBuffer.VerticesPtr[VerticeIndice+2];
    p^.X := FParticleEngine.ParticleList[i].x + 0.5 * epsilon;
    p^.Y := FParticleEngine.ParticleList[i].y + 0.5 * epsilon;
    p^.Z := 0;
    ...
    p := Data.VertexBuffer.VerticesPtr[VerticeIndice+3];
    p^.X := FParticleEngine.ParticleList[i].x - 0.5 * epsilon;
    p^.Y := FParticleEngine.ParticleList[i].y + 0.5 * epsilon;
    p^.Z := 0;
...
constructor TeParticle2d.Create(AOwner: TComponent);
begin
  inherited;
  FParticleEngine := Nil;
  FParticleCount := 100;
...
begin
  ...
  Data.VertexBuffer.Length := FParticleCount * 4;
  Data.IndexBuffer.Length  := FParticleCount * 6;
...
procedure TeParticle2d.Notification(Sender: TObject);
...
begin
  ...
  begin
    ...
    begin
      ...
      Data.VertexBuffer.TexCoord0[fid] := Pointf( SourceConfiguration.TextureData.TextureAtlasItems[ii].TextureSourceX1,
                                                  SourceConfiguration.TextureData.TextureAtlasItems[ii].TextureSourceY1);
      Data.VertexBuffer.TexCoord0[fid+1] := Pointf( SourceConfiguration.TextureData.TextureAtlasItems[ii].TextureSourceX2,
                                                    SourceConfiguration.TextureData.TextureAtlasItems[ii].TextureSourceY1);
      Data.VertexBuffer.TexCoord0[fid+2] := Pointf( SourceConfiguration.TextureData.TextureAtlasItems[ii].TextureSourceX2,
                                                    SourceConfiguration.TextureData.TextureAtlasItems[ii].TextureSourceY2);
      Data.VertexBuffer.TexCoord0[fid+3] := Pointf( SourceConfiguration.TextureData.TextureAtlasItems[ii].TextureSourceX1,
                                                    SourceConfiguration.TextureData.TextureAtlasItems[ii].TextureSourceY2);
      fid := fid + 4;
    end;
    //InternalBuild;
